...
const LANDSCAPE_ASPECT = "600 / 340";

function BlogGridThumb({ item }: { item: Blog }) {
  const [broken, setBroken] = useState(false);
  const showPlaceholder = broken || !item.coverImage;

  return (
    <Link href={`/blog-details/${item._id}`} style={{ display: "block" }}>
      <div
        className="tg-blog-grid-thumb-wrap"
        style={{
          aspectRatio: LANDSCAPE_ASPECT,
          overflow: "hidden",
          background: showPlaceholder ? "#e2e8f0" : "transparent",
          display: "flex",
          alignItems: "center",
...
const BlogArea = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetchBlogs({ limit: 100, status: "published" });
        ...
        <div className="container">
          <div className="row">
            <div className="col-12 text-center py-5">
              <p className="mb-0">Loading blogs…</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="tg-blog-grid-wrap tg-blog-lg-spacing mr-50">
              <div className="row">
                {currentItems.length === 0 ? (
                  <div className="col-12 text-center py-5">
                    <p className="mb-0">No published blogs yet.</p>
                  </div>
                ) : (
                      ...
                      <div className="tg-blog-grid-item mb-30">
                        ...
                        <div className="tg-blog-standard-content">
                          ...
                          <p className="mb-20 tg-blog-standard-para">
                            {item.excerpt ? item.excerpt : stripHtml(item.content)}
                          </p>
                          <div className="tg-blog-sidebar-btn">
                            <Link href={`/blog-details/${item._id}`} className="tg-btn tg-btn-switch-animation">
